...
int handle_request(int clientSocket, ParsedRequest *request, char *tempReq)
{
    ...
    if (cache_buffer && total_size > 0)
        add_cache_element(cache_buffer, total_size, tempReq);
...
void *thread_fn(void *socketNew)
{
    ...
    while (bytes_recv > 0)
    { // http request not comes complete in one time it comes in the form of chunks then it check that complete request comes or not
    ...
    {                                                    // cache miss
        ...
        if (ParsedRequest_parse(request, buffer, strlen(buffer)) == 0)
        {
            ...
            { // check is it get method
                if (request->host && request->path && request->version)
                { // check is it correct url and http version
...
{ //./proxy_server 8080 (./proxy_server - argv[0] , 8080 - argv[1] )
